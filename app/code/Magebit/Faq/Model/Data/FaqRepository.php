<?php
/**
 * This file is part of the Magebit_Faq package.
 * DISCLAIMER
 * Do not edit or add to this file if you wish to upgrade Magebit_Faq
 * to newer versions in the future.
 *
 * @copyright Copyright (c) 2022 Magebit, Ltd. (https://magebit.com/)
 * @author    Magebit <info@magebit.com>
 * @license   MIT
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 */

declare(strict_types=1);

namespace Magebit\Faq\Model\Data;

use Exception;
use Magebit\Faq\Api\Data\FaqInterface;
use Magebit\Faq\Api\Data\FaqSearchResultInterface;
use Magebit\Faq\Api\FaqRepositoryInterface;
use Magebit\Faq\Model\FaqFactory;
use Magebit\Faq\Model\ResourceModel\Faq\Collection;
use Magento\Framework\Api\SearchCriteria\CollectionProcessorInterface;
use Magento\Framework\Api\SearchCriteriaInterface;
use Magento\Framework\Exception\AlreadyExistsException;
use Magento\Framework\Exception\NoSuchEntityException;

class FaqRepository implements FaqRepositoryInterface
{
    /**
     * @var FaqFactory
     */
    private FaqFactory $faqFactory;

    /**
     * @var Collection
     */
    private Collection $faqCollectionFactory;

    /**
     * @var FaqSearchResultInterface
     */
    private FaqSearchResultInterface $faqSearchResultFactory;

    /**
     * @var CollectionProcessorInterface
     */
    private CollectionProcessorInterface $collectionProcessor;

    /**
     * @param FaqFactory $faqFactory
     * @param Collection $faqCollectionFactory
     * @param FaqSearchResultInterface $faqSearchResultFactory
     * @param CollectionProcessorInterface $collectionProcessor
     */
    public function __construct(
        FaqFactory                   $faqFactory,
        Collection                   $faqCollectionFactory,
        FaqSearchResultInterface     $faqSearchResultFactory,
        CollectionProcessorInterface $collectionProcessor
    ) {
        $this->faqFactory = $faqFactory;
        $this->faqCollectionFactory = $faqCollectionFactory;
        $this->faqSearchResultFactory = $faqSearchResultFactory;
        $this->collectionProcessor = $collectionProcessor;
    }

    /**
     * @inheritDoc
     */
    public function getById(int $id): FaqInterface
    {
        $faq = $this->faqFactory->create();
        $faq->getResource()->load($faq, $id);

        if (!$faq->getId()) {
            throw new NoSuchEntityException(__('Unable to find FAQ with entity_id "%1"', $id));
        }

        return $faq;
    }

    /**
     * @inheritDoc
     * @throws AlreadyExistsException
     */
    public function save(FaqInterface $faq): FaqInterface
    {
        $faq->getResource()->save($faq);

        return $faq;
    }

    /**
     * @inheritDoc
     */
    public function getList(SearchCriteriaInterface $searchCriteria): FaqSearchResultInterface
    {
        $collection = $this->faqCollectionFactory->create();
        $this->collectionProcessor->process($searchCriteria, $collection);
        $searchResults = $this->faqSearchResultFactory->create();

        $searchResults->setSearchCriteria($searchCriteria);
        $searchResults->setItems($collection->getItems());

        return $searchResults;
    }

    /**
     * @inheritDoc
     * @throws Exception
     */
    public function delete(FaqInterface $faq): void
    {
        $faq->getResource()->delete($faq);
    }

    /**
     * @inheritDoc
     * @throws NoSuchEntityException
     */
    public function deleteById(int $id): void
    {
        $faq = $this->faqCollectionFactory->getItemById($id);

        if (!isset($faq) || !$faq->getId()) {
            throw new NoSuchEntityException(__('Unable to find FAQ with entity_id "%1"', $id));
        }

        $faq->getResource()->delete();
    }
}
